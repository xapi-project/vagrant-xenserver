...
module VagrantPlugins
  module XenServer
    module Xe
      # Taken from https://gist.github.com/jkutner/1647275
      ...
      class XEViaSSH
        ...
        def initialize(*command)
          @logger  = Log4r::Logger.new("vagrant::xenserver::util::exec")
          @options = command.last.is_a?(Hash) ? command.pop : {}
          xs_username = @options[:machine].provider_config.xs_username
          xs_host = @options[:machine].provider_config.xs_host
          @command = ["ssh", "-l", xs_username, xs_host, "xe"] + command
